...
#define DRIVER_DESC "Device driver tinh"
#define MEM_SIZE 1024

dev_t dev_number = 0;
static struct class *device_class;
static struct cdev *lab42_cdev;
...
static int __init char_driver_init(void)
{
    ...
    dev_number = MKDEV(200,0);
    register_chrdev_region(dev_number, 1, "lab4.2_Hoang_Van_Thai");
    printk("Insert character driver successfully. major(%d), minor(%d)\n", MAJOR(dev_number), MINOR(dev_number));

    device_class = class_create(THIS_MODULE, "Lab042");
    device_create(device_class, NULL, dev_number, NULL,"lab42");

    kernel_buffer = kmalloc(MEM_SIZE , GFP_KERNEL);

    lab42_cdev = cdev_alloc();//cap phat bo nho cho bien cau truc cdev
	cdev_init(lab42_cdev, &fops); //khoi tao cho bien cau truc cdev
	cdev_add(lab42_cdev, dev_number, 1); //dang ky cau truc cdev voi kernel
